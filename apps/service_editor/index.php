...
{	
    ...
    if(isset($_POST['delete'])) {
		...
		removeService($_POST['serviceName']);	//remove the service from systemctl
    }
    if(isset($_POST['update'])) {
        $dbConstructor->updateRow_services($_POST['id'], $_POST['en'], $_POST['serviceName'],
			SQLite3::escapeString($_POST['new_cmd_line']), SQLite3::escapeString($_POST['unitOptions']),
        ...
        createService($_POST['serviceName'], $enable, $_POST['serviceNameOld'], $_POST['new_cmd_line'], SQLite3::escapeString($_POST['unitOptions']),
	...
	if(isset($_POST['loadConfig']))
	{
		...
		foreach ($xml->item as $item) :		 
			...
			editServiceFile($name, $item->cmdline, SQLite3::escapeString($item->unit_options),
			SQLite3::escapeString($item->service_options), SQLite3::escapeString($item->install_options));	 
                            ...
                            <?php foreach ($rows as $index=>$row) : ?>
                                <tr>
                                    ...
                                    <td width='1%' white-space='nowrap'>
                                        <button type='button' class='btn btn-primary btn-sm' title="Click to test the Service" data-bs-toggle='modal' data-bs-target='#testModal' data-bs-id="<?php echo $row['id'];?>" data-bs-name="<?php echo urlencode($row['name']);?>">
                                        Manual</button>
                                    </td>
                                    <td width='1%' white-space='nowrap'>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
					...
					<div class="mb-3">
						...
						<textarea type="text" class="form-control bg-dark text-info fs-5" id="recipient-status" placeholder="Press to test the service" rows="10"></textarea>
					</div>
...
function testService()
{
    ...
    serviceName = document.getElementById('testModal-name').value;
    $.ajax({
        type : "POST",  //type of method
...
function startService()
{
    ...
    $.ajax({
        type : "POST",  //type of method
        url  : "helper/serviceHandler.php",  //your page
        data: "startService=1&serviceName=" + serviceName + "&en=" + en,
        success: function(msg)
        {			
			document.getElementById('recipient-status').value = msg;
        },
...
function stopService()
{
    ...
    $.ajax({
        type : "POST",  //type of method
        url  : "helper/serviceHandler.php",  //your page
        data: "stopService=1&serviceName=" + serviceName,
        success: function(msg)
        {			
			document.getElementById('recipient-status').value = msg;
        },
...
function deleteRow()
{
    ...
    $.ajax({
        type : "POST",  //type of method
        url  : "index.php",  //your page
        data: "delete=1&id=" + id + "&serviceName=" + serviceName,
        success: function(msg)
        {
            location.reload(true);
        },
...
function updateRow()
{
    ...
    $.ajax({
        type : "POST",  //type of method
        url  : "index.php",  //your page
        data: "update=1&id=" + id + "&en=" + en + "&serviceName=" + serviceName + "&serviceNameOld=" + serviceNameOld
		+ "&new_cmd_line=" + new_cmd_line + "&unitOptions=" + unitOptions + "&serviceOptions=" + serviceOptions + "&installOptions=" + installOptions + "&description=" + description,
        success: function(msg)
        {
            location.reload(true);
        },
